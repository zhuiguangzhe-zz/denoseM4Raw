    ...
    def train(self):
        ...
        for i in range(self.args.start_iter, self.args.max_iter):
            ...
            for j, batch_samples in enumerate(self.train_dataloader):
                ...
                ## optimization
                # loss = 0
                # 初始化 loss 变量为张量，并确保 requires_grad 为 True
                loss = torch.tensor(0.0, dtype=torch.float32, device=output.device, requires_grad=True)
                ...
                if self.args.loss_l1:  # 一般情况下，默认L1 loss
                ...
                # print(f"mse_loss: {mse_loss if self.args.loss_mse else 'N/A'}")
                # print(f"l1_loss: {l1_loss if self.args.loss_l1 else 'N/A'}")
                # print(f"adv_loss: {adv_loss if self.args.loss_adv else 'N/A'}")
